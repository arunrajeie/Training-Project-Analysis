function [ ] = analyzeStrategyPT( results, plotFigs, exportFigs )
%ANALYZESTRATEGY Performs analysis on correlatation between mean confidence
...
for sub = 1:numel(subjects)
    ...
    for sesh = 1:10
        session = sprintf('session_%.2d',sesh);
        lastSession = sprintf('session_%.2d', sesh-1);
        if sesh > 1
...
if plotFigs
    ...
    strategyMetaDTimeCoursePlot = figure;
    set(gcf, 'position', [200 200 900 300]);
    subplot(1,2,1); % Control Group
    lineProps = struct('color',{'cm'},'width',3,'edgestyle',':');
    H = mseb(1:10, [meanConfDerivative.group_1.all.mean', metaDaDerivative.group_1.all.mean']', [meanConfDerivative.group_1.all.sem', metaDaDerivative.group_1.all.sem']',lineProps,0); hold on;
    ...
    xlim([0 11]); ylim([ -.5 1]);
    legend('Confidence', 'meta-d''', 'location', 'nw');
    legend boxoff; box off;
    subplot(1,2,2); % Experimental Group
    H = mseb(1:10, [meanConfDerivative.group_2.all.mean', metaDaDerivative.group_2.all.mean']', [meanConfDerivative.group_2.all.sem', metaDaDerivative.group_2.all.sem']',lineProps,0); hold on;
    plot([1 10], [0 0], '--k', 'linewidth', 1);
    set(gca,'fontsize',14);
    set(gca,'xtick', 1:10, 'xticklabel', {'Pre', 2:9, 'Post'});
    ...
    legend boxoff; box off;
    if exportFigs
        export_fig strategyMetaDTimeCoursePlot -png -transparent 'strategyMetaDTimeCourse.png';
    end
    ...
    subplot(1,2,1); % Control Group
    lineProps = struct('color',{'cm'},'width',3,'edgestyle',':');
    H = mseb(1:10, [meanConfDerivative.group_1.all.mean', M_ratioDerivative.group_1.all.mean']', [meanConfDerivative.group_1.all.sem', M_ratioDerivative.group_1.all.sem']',lineProps,0); hold on;
    plot([1 10], [0 0], '--k', 'linewidth', 1);
    set(gca,'fontsize',14);
    set(gca,'xtick', 1:10, 'xticklabel', {'Pre', 2:9, 'Post'});
    title('Control Group');
    ...
    legend boxoff; box off;
    if exportFigs
        export_fig strategyMRatioTimeCoursePlot -png -transparent 'strategyMRatioTimeCourse.png';
    end
    
    strategyMetaDPeakBar = figure;
    set(gcf, 'position', [200 200 450 300]);
    barwitherr([meanConfDerivativePeak.group_2.idx.sem, metaDaDerivativePeak.group_2.idx.sem], [meanConfDerivativePeak.group_2.idx.mean, metaDaDerivativePeak.group_2.idx.mean]);
    set(gca,'fontsize',14);
    set(gca,'xticklabel',{'Confidence', 'meta-d'''}, 'ytick', 2:10, 'yticklabel', {2:9, 'Post'});
    title('Experimental Group');
    xlabel('Measures'); ylabel('Session Peak');
    xlim([0 3]); ylim([1 10]);
    [h, p, ci, stats] = ttest(meanConfDerivativePeak.group_2.idx.raw, metaDaDerivativePeak.group_2.idx.raw);
    t = text(.33,5.5,sprintf('t(%d) = %.3f\np < 0.001', stats.df, stats.tstat));
    set(t, 'fontsize', 14);
    box off;
    if exportFigs
        export_fig strategyMetaDPeakBar -png -transparent 'strategyMetaDPeak.png';
    end
    ...
    t = text(.3,5.5,sprintf('t(%d) = %.3f\np < 0.001', stats.df, stats.tstat));
    set(t, 'fontsize', 14);
    box off;
    if exportFigs
        export_fig strategyMRatioPeakBar -png -transparent 'strategyMRatioPeak.png';
    end
